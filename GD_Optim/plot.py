...
def plotSingle(model, first_param_grid, sampled_params, sampled_loss, nextSample, second_param_grid=None,
                param_dims_to_plot=[0, 1], file_path=None, optimum=None, iteration=0):
    print("FUCK")
    """ plot_iteration
    ...
    if second_param_grid is None:
        ...
        ei = -1 * expectedImprovement(first_param_grid, model, sampled_loss[:],
                                       greater_is_better=True, n_params=1)

        fig, ax1, ax2 = _plot_loss_1d(first_param_grid, sampled_params[:, :], sampled_loss[:], mu, std, ei, nextSample, yerr=alpha, true_y=true_y)
    else:
        ...
        ei = -1 * expectedImprovement(param_grid, model, sampled_loss[:],
                                       greater_is_better=True, n_params=2)

        fig, ax1, ax2 = _plot_loss_2d(first_param_grid, second_param_grid, sampled_params[:, param_dims_to_plot], sampled_loss, mu, ei, nextSample, optimum)

    if file_path is not None:
        plt.savefig('%s/bo_iteration_%d.png' % (file_path, iteration), bbox_inches='tight')
