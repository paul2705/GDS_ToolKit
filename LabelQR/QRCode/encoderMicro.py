...
def makeLabelWithQRCode(text, height, align, QRSize=None, layer=20, QRSavePath=None):
    """An extended function to render a text with its corresponding Micro QRCode 
    NOTE: Micro QRCode can ONLY hold least than 20 characters!
    ...
    with nd.Cell(f'label_{text}') as labelWithQRCode:
        ...
        for y in range(0, N):
            ...
            for x in range(0, M):
                ...
                if value != on:
                    ...
                    if on: 
                        ...
                        xl = xstart * pixelSize
                        xr = int(np.max([x, xstart])) * pixelSize
                        yl = -y * pixelSize
                        yr = (-y - 1) * pixelSize
            ...
            if on: 
                ...
                xl = xstart * pixelSize
                xr = int(np.max([M, xstart])) * pixelSize
                yl = -y * pixelSize
                yr = (-y - 1) * pixelSize
                nd.Polygon(layer=layer, points=[(xl, yl), (xr, yl), (xr, yr), (xl, yr)]).put(startPoint)
